...
stop_frisk_log %>% 
  ...
  count(race) %>% 
  mutate(
    percent_innocent = (n/nrow(under_18)* 100),
    race = fct_reorder(race, -n)) %>% 
  plot_ly(x = ~ race, y = ~ percent_innocent, type = "bar", color = ~ race)
...
# percent innocent in that race category
stop_frisk_log %>% 
  filter(age < 18,
         race == 'black') %>% 
...
output = map(stop_frisk_df$race, under_18_inn)
```
